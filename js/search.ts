...
export const initializeSearchHighlighter = (): void => {
    ...
    searchForm.addEventListener("submit", (e: SubmitEvent): void => {
        ...
        document.querySelectorAll<HTMLElement>(".highlight").forEach((el: HTMLElement): void => {
        ...
        const searchInput = target.querySelector<HTMLInputElement>('input[name="q"]');
        ...
        const walk = (node: Node): void => {
            if (node.nodeType === NODE_TYPE.TEXT) {
                ...
                if (article && article.contains(element)) {
